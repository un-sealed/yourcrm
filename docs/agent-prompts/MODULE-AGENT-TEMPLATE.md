...
4. **Do not run `gen:barrels` or `gen:routes`, and never hand-edit a barrel.**
   `packages/crm/src/index.ts`, `packages/database/src/schema/index.ts` and
   `apps/api/src/routes/modules/index.ts` are generated, centrally owned, and
   regenerated by the integrator after your branch merges. Leave them alone —
   your module folder will be picked up automatically.
