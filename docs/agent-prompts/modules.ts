...
export const WAVE_2: ModuleSpec[] = [
  ...
  },
  {
    slug: "quotes",
    title: "Quotes",
    specFile: "19-quotes.md",
    migrationSlot: "0130",
    permissionObject: "quote",
    tables: ["quotes", "quote_line_items"],
    events: ["quote.created", "quote.updated", "quote.sent", "quote.accepted", "quote.rejected"],
    p0Extra: [
      "- Line items referencing products by plain uuid (Products agent owns that table)",
      "- Totals: subtotal, discount, tax, grand total — computed server-side, never trusted from the client",
      "- Status lifecycle draft → sent → accepted/rejected, each transition emitting its event",
      "- PDF generation is OUT of scope for P0",
    ],
    softRefs: ["products", "deals", "companies", "people"],
  },
  {
    slug: "invoices",
    title: "Invoices & Payments",
    specFile: "20-invoices-payments.md",
    migrationSlot: "0140",
    permissionObject: "invoice",
    tables: ["invoices", "invoice_line_items", "payments"],
    events: ["invoice.created", "invoice.updated", "invoice.sent", "invoice.paid", "payment.recorded"],
    p0Extra: [
      "- Line items + manually recorded payments; no payment-gateway integration in P0",
      "- Balance due derived from payments, never stored as a mutable field",
      "- Overdue detection by due date",
      "- `quote_id` is a plain uuid — the Quotes agent owns that table",
    ],
    softRefs: ["quotes", "companies", "people"],
  },
  {
    slug: "calendar",
    title: "Calendar",
    specFile: "13-calendar.md",
    migrationSlot: "0150",
    permissionObject: "calendar_event",
    tables: ["calendar_events", "calendar_event_attendees"],
    p0Extra: [
      "- Internal events only. **No Google/Microsoft sync in P0** — that needs OAuth",
      "  and an integrations framework neither of which exists yet. Do not start it.",
      "- Month and agenda views; all-day and timed events; attendee list",
      "- Store UTC, render in the workspace timezone",
    ],
    events: ["calendar.event_created", "calendar.event_updated", "calendar.event_deleted"],
    softRefs: ["people", "companies", "deals"],
  },
  {
    slug: "reports",
    title: "Reports",
    specFile: "26-reports.md",
    migrationSlot: "0160",
    permissionObject: "report",
    tables: ["reports"],
    events: ["report.created", "report.updated", "report.run"],
    p0Extra: [
      "- A report is a saved definition: object type, filter tree, grouping, aggregation",
      "- Execute against existing module APIs; results MUST be permission-filtered",
      "  for the calling actor, not merely workspace-scoped",
      "- Table output only in P0; charts come with Dashboards",
      "- Reuse the filter tree type from @yourcrm/ui — do not define a second one",
    ],
    softRefs: [],
  },
  {
    slug: "dashboards",
    title: "Dashboards",
    specFile: "27-dashboards.md",
    migrationSlot: "0170",
    permissionObject: "dashboard",
    tables: ["dashboards", "dashboard_widgets"],
    events: ["dashboard.created", "dashboard.updated"],
    p0Extra: [
      "- Grid layout of widgets; each widget references a report by plain uuid",
      "- Widget types: metric tile, table, bar, line",
      "- Layout persisted per dashboard; drag-to-reposition",
      "- Empty state must explain how to create the first widget",
    ],
    softRefs: ["reports"],
  },
]
