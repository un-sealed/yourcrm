...
## 1. Purpose
Capture, qualify, route and convert inbound/outbound prospects.

## 2. Routes
/app/leads, /app/leads/:id

## 3. Scope / Feature Checklist
- [ ] Lead inbox/table with status, source, score, owner, created date and response SLA.
- [ ] Lead detail with source metadata, UTM parameters, form answers, timeline and qualification fields.
- [ ] Lead lifecycle statuses: new, contacted, qualified, unqualified, converted, recycled.
- [ ] Lead source tracking for forms, Meta, Google, WhatsApp, IndiaMART, JustDial, TradeIndia and manual entry.
- [ ] Qualification fields and configurable qualification framework.
- [ ] Round-robin and rule-based assignment.
- [ ] Duplicate matching against people and companies.
- [ ] Convert lead into person/company/deal without duplicate creation.
- [ ] Bulk qualification, assignment, tagging and export.
- [ ] Follow-up tasks and sequence enrollment.
- [ ] AI lead summary, qualification suggestions, routing recommendation and next action.
- [ ] Spam/fraud scoring hook.
- [ ] Mobile lead capture and call/WhatsApp actions.
...
## 6. Data Model
Lead, LeadSource, LeadStatus, LeadScore, LeadConversion, UTM, FormSubmission.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
CRUD, qualify, convert, assign, score, import, source ingestion, bulk actions.
...
## 8. Permissions
lead visibility and ownership; conversion; export; source-management permissions.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
lead.created, lead.qualified, lead.assigned, lead.converted, lead.recycled.
...
Capture, dedupe, qualify, assign, convert, audit and automation triggers all work.
