...
## 1. Purpose
Make the CRM usable from Claude, ChatGPT and other MCP-capable clients.
...
- [ ] Built-in MCP server.
- [ ] Read tools for records, search, timelines, reports and metadata.
- [ ] Write tools for create/update tasks, notes, records and approved actions.
...
- [ ] Rate limits.
- [ ] Audit logging.
- [ ] Tool schemas with descriptions/examples.
- [ ] Resource endpoints for knowledge/context where appropriate.
- [ ] Self-hosted and cloud support.
- [ ] Connection management and revoke.
- [ ] Usage logs.
...
## 6. Data Model
MCPClient, MCPToken, MCPPolicy, MCPAuditEvent.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
OAuth, tool discovery, tool execution, resource access.
...
## 8. Permissions
never exceed authenticated user/agent scopes.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
mcp.connected, mcp.tool_called, mcp.write_approved, mcp.disconnected.
...
An external AI client can safely search and perform approved CRM actions.
