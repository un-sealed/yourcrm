...
## 1. Purpose
Identity-to-access control model shared by every module.

## 2. Routes
/app/settings/users, /app/settings/teams, /app/settings/roles
...
## 6. Data Model
User, Team, TeamMember, Role, Permission, RecordAccessRule.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
users, invitations, teams, roles, permission checks.
...
## 8. Permissions
workspace admin for configuration.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
user.invited, user.activated, user.deactivated, team.member_added, role.updated.
...
A permission matrix can be tested against every domain module.
