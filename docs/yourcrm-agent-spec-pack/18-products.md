...
## 1. Purpose
Catalog products/services used in deals, quotes and invoices.

## 2. Routes
/app/products, /app/products/:id, /app/products/price-books

## 3. Scope / Feature Checklist
- [ ] Product catalog with SKU, name, description, unit, price, cost, tax category and active status.
- [ ] Categories and tags.
- [ ] Multiple price books.
- [ ] Currency-specific prices.
- [ ] Discount rules.
- [ ] Tax rules.
- [ ] Product search and bulk import/export.
- [ ] Archive instead of destructive deletion.
- [ ] Attach products to deals.
- [ ] Future inventory integration hook; do not build full ERP inventory.
- [ ] AI product matching and quote suggestions hook.
...
## 6. Data Model
Product, ProductCategory, PriceBook, PriceBookItem, TaxRule.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
CRUD, price lookup, bulk import/export, archive.
...
## 8. Permissions
catalog admin, read, edit, price visibility.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
product.created, product.updated, price.changed, product.archived.
...
Deals and quotes can reference stable product/price data.
