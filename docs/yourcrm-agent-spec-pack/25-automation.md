...
## 1. Purpose
Visual deterministic automation engine with optional AI steps.

## 2. Routes
/app/automation, /app/automation/:id, /app/automation/runs

## 3. Scope / Feature Checklist
- [ ] Visual workflow builder.
- [ ] Triggers: record created/updated, field changed, scheduled, webhook, form submitted, email received, message received, stage changed.
- [ ] Actions: update record, create task, send email, send WhatsApp, notify user, call webhook, assign owner, add tag, create activity, run AI agent.
- [ ] Conditions and AND/OR branches.
- [ ] Delays and scheduled waits.
- [ ] Loops with explicit limits.
- [ ] Approvals.
- [ ] Retry policy.
- [ ] Run history.
...
- [ ] Versioning and draft/published lifecycle.
- [ ] Idempotency and loop protection.
- [ ] Usage limits.
- [ ] Describe-to-automate AI builder that generates a draft requiring approval.
...
## 6. Data Model
Workflow, WorkflowVersion, WorkflowTrigger, WorkflowStep, WorkflowRun, ApprovalRequest.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
CRUD, validate, publish, run, pause, resume, history.
...
## 8. Permissions
automation admin; send-message permission; AI action scopes.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
workflow.published, workflow.run_started, workflow.step_failed, workflow.completed.
...
A user can create, test, publish and observe reliable automations.
