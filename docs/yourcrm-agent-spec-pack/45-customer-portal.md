...
## 1. Purpose
Give customers controlled access to tickets, quotes, invoices and shared documents.

## 2. Routes
/portal, /portal/tickets, /portal/quotes, /portal/invoices

## 3. Scope / Feature Checklist
- [ ] Magic-link or secure customer authentication.
- [ ] Customer profile.
...
## 6. Data Model
PortalUser, PortalSession, PortalAccessRule, PortalEvent.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
portal auth, tickets, quotes, invoices, files, payments.
...
## 8. Permissions
explicit customer-visible access only.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
portal.login, portal.quote_accepted, portal.ticket_created.
...
Internal-only CRM data never leaks into customer portal.
