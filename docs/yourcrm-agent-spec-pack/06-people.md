...
## 1. Purpose
Contact/individual 360-degree management.

## 2. Routes
/app/people, /app/people/:id

## 3. Scope / Feature Checklist
- [ ] Table view with configurable columns, inline edit, filters, grouping, sorting, saved views and bulk actions.
- [ ] Person detail with identity, contact methods, company, owner, tags, custom fields, relationships and full timeline.
- [ ] Create/edit person with duplicate detection before save.
- [ ] Multiple emails, phone numbers, addresses and preferred communication channel.
- [ ] Person-to-company roles and primary-company relationship.
- [ ] Merge duplicate people with field-by-field conflict resolution and audit trail.
- [ ] Import/export CSV/Excel/JSON with mapping, preview and duplicate handling.
- [ ] Activity composer for note, task, call, meeting, email and attachment.
- [ ] Internal comments and mentions.
- [ ] Enrichment hooks for future web research and AI fields.
- [ ] Relationship strength, warm-intro and buying-signal hooks for later phases.
- [ ] GDPR/DPDP consent and communication preferences.
- [ ] Mobile quick actions: call, WhatsApp, email, note, task.
- [ ] AI summary, next-step suggestions and automatic extraction from conversations.
...
## 6. Data Model
Person, PersonEmail, PersonPhone, Address, PersonCompanyRole, Tag, Relationship, Consent, custom fields.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
CRUD, search, merge, bulk update, import/export, timeline, relationships, enrichment.
...
## 8. Permissions
read/create/edit/delete people; field-level permissions; owner/team/all visibility; export permission.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
person.created, person.updated, person.merged, person.deleted, person.relationship_changed.
...
Create/update/delete/restore; duplicate detection; merge; timeline; filters; bulk actions; import/export; mobile actions; audit events.
