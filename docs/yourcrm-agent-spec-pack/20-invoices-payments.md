...
## 1. Purpose
Lightweight CRM invoicing/payment links without becoming an ERP.

## 2. Routes
/app/invoices, /app/invoices/:id, /app/payments

## 3. Scope / Feature Checklist
- [ ] Invoice creation from accepted quote/deal.
- [ ] Customer billing details.
- [ ] Line items, taxes, discounts and currency.
- [ ] Invoice numbering and status.
- [ ] PDF generation.
- [ ] Payment links through Stripe/Razorpay and UPI-compatible regional adapters.
- [ ] Payment status webhook handling.
- [ ] Automatic deal/payment status updates.
- [ ] GST-ready fields and invoice configuration.
- [ ] Credit-note/refund hooks.
...
## 6. Data Model
Invoice, InvoiceLineItem, Payment, PaymentLink, Tax, BillingProfile.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
CRUD, PDF, payment link, provider webhook, export.
...
## 8. Permissions
financial record access, payment administration.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
invoice.created, invoice.sent, payment.created, payment.succeeded, payment.failed.
...
Invoice/payment state is reliable and auditable.
