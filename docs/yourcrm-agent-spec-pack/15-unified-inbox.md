...
## 1. Purpose
Single workspace for email, WhatsApp, SMS, Telegram and website chat.

## 2. Routes
/app/inbox, /app/inbox/:conversationId

## 3. Scope / Feature Checklist
- [ ] Conversation list with unread, assigned, channel, SLA and priority filters.
- [ ] Channel-specific message rendering.
- [ ] Assignment to users/teams.
- [ ] Internal notes separate from customer messages.
- [ ] Conversation tags and status.
- [ ] Customer identity resolution.
- [ ] Attachments, media and interactive messages where supported.
- [ ] Saved replies/templates.
- [ ] Conversation merge/split.
- [ ] SLA timers and escalation hooks.
- [ ] Search across conversations.
- [ ] Bulk assign/archive/mark read.
- [ ] AI suggested replies and summaries.
- [ ] AI intent classification and lead/ticket routing.
- [ ] Human approval before autonomous external sending.
...
## 6. Data Model
Conversation, ConversationParticipant, Message, MessageAttachment, Assignment, ConversationTag.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
conversation CRUD, message send, assignment, channel adapters, search.
...
## 8. Permissions
channel access, assignment, team visibility, send permission.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
message.received, message.sent, conversation.assigned, conversation.closed.
...
At least email + one messaging channel can share a common conversation model.
