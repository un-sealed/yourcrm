...
## 1. Purpose
Configure reusable sales/process pipelines.

## 2. Routes
/app/settings/pipelines, /app/pipelines/:id

## 3. Scope / Feature Checklist
- [ ] Create multiple pipelines for sales, partnerships, hiring or custom processes.
- [ ] Stage CRUD with name, order, probability, color/icon, required fields and terminal behavior.
- [ ] Pipeline-specific fields and validation.
- [ ] Default pipeline per workspace/team.
- [ ] Stage automation triggers.
- [ ] Pipeline conversion analytics.
- [ ] Archive pipeline without deleting historical deals.
- [ ] Import/export configuration.
- [ ] Pipeline templates.
- [ ] AI setup wizard can generate a proposed pipeline for approval.
...
## 6. Data Model
Pipeline, PipelineStage, StageRequirement, PipelineTeamAssignment.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
CRUD, reorder, activate/archive, stage validation and analytics.
...
## 9. Domain Events
pipeline.created, pipeline.updated, pipeline.stage_created, pipeline.stage_changed.
...
Admin can create and configure pipelines without code.
