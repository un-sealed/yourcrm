# Plugin / App SDK & Marketplace

## 1. Purpose
Allow developers to extend YourCRM safely.

## 2. Routes
/app/settings/apps, /app/marketplace
...
- [ ] Revenue-share hook for hosted marketplace.
- [ ] Developer documentation and example apps.
...
## 6. Data Model
App, AppInstallation, AppPermission, AppVersion, AppSecret.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
install, uninstall, manifest, permissions, extension APIs.
...
## 8. Permissions
workspace admin; app scopes constrained.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
app.installed, app.uninstalled, app.error.
...
A sample external app can add a field/action/integration without modifying core code.
