...
## 1. Purpose
Bring email into CRM without requiring manual logging.

## 2. Routes
/app/email, /app/email/settings

## 3. Scope / Feature Checklist
- [ ] Gmail and Outlook OAuth two-way sync.
- [ ] IMAP/SMTP for other providers.
- [ ] Send email from record pages and compose screen.
- [ ] Threading and quoted-message handling.
- [ ] Automatic matching by email address and domain.
- [ ] Manual link/unlink to records.
- [ ] Email templates with variables.
- [ ] Attachments.
- [ ] Open/click tracking as opt-in and privacy-aware.
- [ ] Scheduled send.
- [ ] Signatures.
- [ ] Aliases/shared mailboxes hook.
- [ ] BCC logging address.
- [ ] Sequence integration.
- [ ] Email delivery/retry/error status.
- [ ] AI draft, summarize, classify, extract next steps and suggest record updates.
- [ ] PII/privacy controls and retention settings.
...
## 6. Data Model
EmailAccount, EmailMessage, EmailThread, EmailAttachment, EmailTemplate, TrackingEvent.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
sync, threads, send, templates, tracking, link/unlink, search.
...
## 8. Permissions
mailbox connection and send-as permissions; private/shared mailbox controls.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
email.received, email.sent, email.thread_linked, email.bounced.
...
Users can connect Gmail/Outlook, see linked threads, send from CRM and preserve auditability.
