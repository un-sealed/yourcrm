...
## 1. Purpose
Developer-facing programmable interface.

## 2. Routes
/app/settings/api, /docs/api

## 3. Scope / Feature Checklist
- [ ] REST API with OpenAPI.
- [ ] GraphQL API where justified.
- [ ] API keys with scopes.
- [ ] OAuth application support later.
...
- [ ] Sandbox/test workspace support later.
...
## 6. Data Model
ApiKey, OAuthApp, WebhookEndpoint, WebhookDelivery, RateLimitBucket.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
documented externally; admin UI for keys/webhooks.
...
## 8. Permissions
API key scopes must not exceed creator permissions.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
webhook.delivery_succeeded, webhook.delivery_failed, api_key.created.
...
Developers can integrate safely without direct DB access.
