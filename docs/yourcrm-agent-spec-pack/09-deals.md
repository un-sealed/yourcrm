...
## 1. Purpose
Manage revenue opportunities from qualification through close.

## 2. Routes
/app/deals, /app/deals/:id

## 3. Scope / Feature Checklist
- [ ] Table and Kanban views with drag-and-drop stages.
- [ ] Deal detail with value, currency, expected close, pipeline, stage, probability, owner and account/person relationships.
- [ ] Multiple pipelines and custom stage configuration.
- [ ] Stage transition validation and optional required fields.
- [ ] Won/lost reasons and competitor fields.
- [ ] Stale/rotting deal detection based on inactivity threshold.
- [ ] Deal health indicators based on activity, close date, stage age and missing fields.
- [ ] Products/line items hook for quotes and revenue.
- [ ] Next-step task and reminder.
- [ ] Deal timeline with all communication and field changes.
- [ ] Internal deal threads and mentions.
- [ ] Bulk stage/owner/value updates with confirmation.
- [ ] Forecast category and expected value.
- [ ] AI deal summary, risk explanation, next-best action and follow-up draft.
- [ ] Digital sales room and payment-link hooks.
...
## 6. Data Model
Deal, DealStage, DealParticipant, DealLineItem, ForecastCategory, WonLostReason.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
CRUD, stage move, forecast, line items, merge, timeline, bulk actions.
...
## 8. Permissions
object/record/field permissions; stage transition permissions; export.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
deal.created, deal.updated, deal.stage_changed, deal.won, deal.lost, deal.stale.
...
Create, move, close, forecast, stale detection, timeline, bulk actions and AI hooks.
