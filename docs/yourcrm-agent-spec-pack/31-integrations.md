...
## 1. Purpose
Manage external connections and provider adapters.
...
- [ ] Supported categories: email, calendar, WhatsApp, calling, payments, accounting, Slack/Teams, Zapier/n8n/Make, analytics/product usage.
- [ ] India-specific: IndiaMART, JustDial, TradeIndia, Tally, Zoho Books, Exotel, Knowlarity, MyOperator.
...
## 6. Data Model
Integration, Connection, CredentialReference, SyncJob, SyncError, FieldMapping.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
connect, OAuth callback, disconnect, sync, health, mapping.
...
## 8. Permissions
integration admin only unless delegated.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
integration.connected, integration.disconnected, sync.started, sync.failed.
...
Integrations have consistent lifecycle, observability and secret handling.
