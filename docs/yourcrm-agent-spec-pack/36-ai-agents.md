...
## 1. Purpose
Safe agents that perform bounded CRM work.

## 2. Routes
/app/ai/agents, /app/ai/agents/:id, /app/settings/ai/agents

## 3. Scope / Feature Checklist
- [ ] Built-in agents: data hygiene, follow-up, research, support, lead qualification.
...
- [ ] Run history.
- [ ] Error/retry.
- [ ] Agent health metrics: errors, latency, escalations and success.
- [ ] Agent can be used as workflow action.
- [ ] Agent can use MCP tools only within inherited scopes.
...
## 6. Data Model
Agent, AgentVersion, AgentToolPermission, AgentRun, AgentAction, ApprovalRequest.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
CRUD, publish, run, approve, cancel, logs, health.
...
## 8. Permissions
agent admin; per-agent scopes; action-level approvals.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
agent.published, agent.run_started, agent.action_requested, agent.action_approved, agent.completed.
...
No agent can perform an unscoped destructive/external action.
