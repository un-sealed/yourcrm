# Observability, Backups & System Health

## 1. Purpose
Keep self-hosted and cloud deployments operable.

## 2. Routes
/app/settings/system, /app/settings/backups, /app/settings/health
...
- [ ] Request IDs.
- [ ] Performance metrics.
- [ ] Agent/automation latency.
- [ ] Scheduled backups.
- [ ] S3/Google Drive backup target hooks.
- [ ] Backup encryption.
- [ ] Restore workflow.
- [ ] Backup verification.
- [ ] Retention policy.
...
## 6. Data Model
HealthCheck, JobRun, BackupJob, BackupTarget, DiagnosticReport.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
health, jobs, retry, backups, restore status.
...
## 9. Domain Events
job.failed, backup.completed, backup.failed, restore.started.
...
Admin can determine system health and verify backups.
