...
## 1. Purpose
Flexible reporting over CRM data.

## 2. Routes
/app/reports, /app/reports/:id

## 3. Scope / Feature Checklist
- [ ] Chart types: bar, line, area, pie, funnel, table, KPI, cohort where feasible.
- [ ] Object/relationship selection.
...
- [ ] AI report builder from natural language with human review.
...
## 6. Data Model
Report, ReportQuery, ReportWidget, ReportSchedule.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
report CRUD, execute, export, schedule.
...
## 8. Permissions
report visibility and underlying-record permissions.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
report.created, report.executed, report.scheduled.
...
Reports never expose records the viewer cannot access.
