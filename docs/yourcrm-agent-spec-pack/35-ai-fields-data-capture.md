# AI Fields & Automatic Data Capture

## 1. Purpose
Reduce manual CRM entry by extracting and enriching structured data.

## 2. Routes
/app/settings/ai-fields, object record pages

## 3. Scope / Feature Checklist
- [ ] AI field definitions per object.
- [ ] Summary, classification, sentiment, score and extraction fields.
- [ ] Web-research fields with source URL and retrieval date.
- [ ] Automatic extraction from email and meeting content.
- [ ] Detect next steps, dates, people, amounts and competitors.
- [ ] Suggest record updates for approval.
- [ ] OCR business cards and screenshots.
- [ ] Human override.
- [ ] Regenerate field.
- [ ] Model/provider selection.
- [ ] Confidence score.
- [ ] Prompt/version tracking.
- [ ] Privacy mode and PII redaction.
- [ ] Batch enrichment with rate/cost limits.
...
## 6. Data Model
AIField, AIFieldRun, ExtractionSuggestion, ResearchSource.

All records must follow the shared conventions in `03-data-model.md`.

## 7. API Requirements
field config, run, approve/reject, batch enrichment.
...
## 8. Permissions
AI configuration admin; record write permission for changes.

Permission checks must occur server-side. UI hiding is not a security boundary.

## 9. Domain Events
ai_field.completed, suggestion.created, suggestion.approved, enrichment.failed.
...
AI suggestions are visible, attributable, reversible and permission-aware.
