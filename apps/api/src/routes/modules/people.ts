...
/**
 * People module (spec 06-people, P0) — the golden reference for module agents.
 *
...
export const basePath = "/people"

const personEnvelope = z.object({ data: personSchema.passthrough() })
const personListEnvelope = paginatedEnvelopeSchema(personSchema.passthrough())

export type PeopleRouteDeps = {
  service?: PeopleService
}

function defaultService(): PeopleService {
  const db = getDb()
...
export function createRoutes(deps: PeopleRouteDeps = {}) {
  ...
  let cached: PeopleService | null = deps.service ?? null
  const service = () => (cached ??= defaultService())

  app.get(
    "/",
    requireSession(),
    zValidator("query", personQuerySchema, (result, c) => {
      if (!result.success) {
        return c.json(
          errorEnvelope(
            "VALIDATION_ERROR",
  ...
  app.get("/:id", requireSession(), async (c) => {
    try {
      ...
      return c.json({ data: { ...found.person, emails: found.emails, phones: found.phones } })
    } catch (err) {
      return mapError(c, err)
    }
  })
  ...
  app.post(
    "/",
    requireSession(),
    zValidator("json", createPersonSchema, (result, c) => {
      if (!result.success) {
        return c.json(
          errorEnvelope(
            "VALIDATION_ERROR",
    ...
    async (c) => {
      try {
        const person = await service().create(serviceContextOf(c), c.req.valid("json"))
  ...
  app.patch(
    "/:id",
    requireSession(),
    zValidator("json", updatePersonSchema, (result, c) => {
      if (!result.success) {
        return c.json(
          errorEnvelope(
            "VALIDATION_ERROR",
  ...
  app.post("/:id/restore", requireSession(), async (c) => {
    try {
      const person = await service().restore(serviceContextOf(c), c.req.param("id"))
      return c.json({ data: person })
    } catch (err) {
      return mapError(c, err)
    }
...
export const openApiPaths = {
  "/api/v1/people": {
    get: {
      summary: "List people (cursor pagination, search, status filter)",
    ...
    post: {
      ...
      requestBody: {
        content: { "application/json": { schema: zodToJsonSchema(createPersonSchema) } },
      },
    },
  },
  "/api/v1/people/{id}": {
    get: { summary: "Get a person with contact methods", operationId: "getPerson" },
    patch: {
      summary: "Update a person",
      operationId: "updatePerson",
      requestBody: {
        content: { "application/json": { schema: zodToJsonSchema(updatePersonSchema) } },
      },
    },
    delete: { summary: "Soft-delete a person", operationId: "deletePerson" },
  },
  "/api/v1/people/{id}/restore": {
    post: { summary: "Restore a soft-deleted person", operationId: "restorePerson" },
  },
}

export { personEnvelope, personListEnvelope }
