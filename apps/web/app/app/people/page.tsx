...
import { displayName, PERSON_FILTER_FIELDS, type PeopleListResponse, type Person } from "./types"
import { treeToPeopleParams } from "./filters"

type SavedViewState = { id: string; name: string; tree: FilterTree; search: string }

const VIEWS_KEY = "yourcrm.people.views"
...
function personColumns(): DataTableColumn<Person>[] {
  return [
    {
      id: "name",
      header: "Name",
      sortable: true,
      accessor: (row) => (
        <Link href={`/app/people/${row.id}`} className="font-medium text-primary hover:underline">
          {displayName(row)}
        </Link>
      ),
    },
    {
      ...
      header: "Title",
      sortable: true,
      accessor: (row) => row.title ?? <span className="text-muted-foreground">—</span>,
      renderEdit: ({ row, onCommit, onCancel }) => (
        <form
          onSubmit={(e) => {
            e.preventDefault()
            const value = new FormData(e.currentTarget).get("title")
            onCommit({ title: typeof value === "string" && value !== "" ? value : null })
          }}
    ...
    },
    {
      id: "channel",
      header: "Channel",
      accessor: (row) => row.preferredChannel ?? <span className="text-muted-foreground">—</span>,
    },
  ]
}
...
export default function PeopleListPage() {
  ...
  const params = useMemo(() => {
    const fromTree = treeToPeopleParams(tree)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetchRaw<PeopleListResponse>(`/api/v1/people?${qs.toString()}`)
      setRows(res.data)
  ...
  const inlineCommit = useCallback(
    async (rowId: string, patch: Partial<Person>) => {
      try {
        const updated = await apiFetch<Person>(`/api/v1/people/${rowId}`, {
          method: "PATCH",
          body: patch,
        })
  ...
  const bulkDelete = async () => {
    ...
    try {
      for (const id of selectedIds) {
        await apiFetchRaw(`/api/v1/people/${id}`, { method: "DELETE" })
      }
      setSelectedIds([])
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Link href="/app/people/new" className={buttonVariants()}>
          New person
        </Link>
      </div>
      ...
      {error !== null ? (
        ...
        <DataTable
          ...
          empty={
            <EmptyState
              title="No people yet"
              description="Add your first contact to start building the pipeline."
              action={
                <Link href="/app/people/new" className={buttonVariants()}>
                  New person
                </Link>
