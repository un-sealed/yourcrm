 ...
 */
export function treeToQuoteParams(tree: FilterTree): QuotesListParams {
  const params: QuotesListParams = {}
  const visit = (node: FilterNode): void => {
    ...
    if (leaf.field === "status" && (leaf.operator === "eq" || leaf.operator === "in")) {
      ...
      if (value === "draft" || value === "sent" || value === "accepted" || value === "rejected") {
        params.status = value
      }
      return
    }
